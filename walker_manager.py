...
class WalkerManager(object):
    ...
    def __init__(self, map, vehicle_actor, walker_list, world):
        """
        Description:
            Method __init__ is the Constructor of Class WalkerManager that initializes most of the used variables 
    ...
    def check_pedestrians(self, waypoints, index):
        """
        Description:
            Method check_pedestrians checks for possible pedestrians at the same, right or left lane 

        Args:
            waypoints   (list)  : The list of waypoints that are crossed by the vehicle 
